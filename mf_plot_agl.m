...
function [hcb] = mf_plot_agl(lon, lat, var, var_sign, caxis_dom, colormap_dom, res, flag_sp, flag_cb, panel, experiment, cbcaption)
...
% Set grid characteristics - CCLM domain
lat_min = -9.75;
lat_max = 2.69;
lon_min = 26;
lon_max = 35.94;
...
m_grid('box','on','tickdir','in','color',[0, 0, 0], 'Fontweight', 'Bold','FontSize', 11, 'linewidth',1.5,'linestyle','none','xtick',[26 30 34 36 38],'ytick',[-10 -8 -6 -4 -2 0 2 4 6 8 10], 'color', axcolor); hold on; % draw grid with normal box
...
% plot color bar
if flag_cb == 1
    hcb = colorbar('Fontsize', 14, 'Fontweight', 'Bold', 'YColor', axcolor); hold on;     
    cbfreeze; mf_freezeColors;
elseif flag_cb == 2
    hcb  = colorbar('location', 'SouthOutside', 'Fontsize', 14, 'Fontweight', 'Bold', 'TickLength', [0 0], 'XColor', axcolor); hold on;     
    y1   = get(gca,'position');
    y    = get(hcb,'Position');
    y(4) = 0.03;
    set(hcb,'Position',y)
...
% add text
m_text(lon_min+0.02, lat_max-0.01, panel,'ver','bottom','hor','left', 'Fontweight', 'Bold', 'Fontsize', 12, 'color', axcolor); hold on;
m_text(lon_max-0.02, lat_max-0.01, experiment,'ver','bottom','hor','right', 'Fontweight', 'Bold', 'Fontsize', 11, 'color', axcolor); hold on;
if flag_cb == 1
    m_text(240,0,cbcaption,'ver','bottom','hor','center', 'Fontweight', 'Bold', 'Fontsize', 12,'rotation',-90, 'color', axcolor); hold off;
elseif flag_cb == 2
%     m_text((lon_min+lon_max)/2,lat_min-5.5,cbcaption,'ver','bottom','hor','center', 'Fontweight', 'Bold', 'Fontsize', 12, 'color', axcolor); hold off;
    m_text((lon_min+lon_max)/2,lat_min-4.5,cbcaption,'ver','bottom','hor','center', 'Fontweight', 'Bold', 'Fontsize', 12, 'color', axcolor); hold off;
end
